#' Conflicts between the cytoverse and other packages
#'
#' This function lists all the conflicts between packages in the cytoverse
...
#' @examples
#' cytoverse_conflicts()
cytoverse_conflicts <- function() {
  envs <- purrr::set_names(search())
  objs <- invert(lapply(envs, ls_env))

  conflicts <- purrr::keep(objs, ~ length(.x) > 1)

  tidy_names <- paste0("package:", cytoverse_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))

  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)

  structure(conflict_funs, class = "cytoverse_conflicts")
}

cytoverse_conflict_message <- function(x) {
  if (length(x) == 0) return("")

  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "cytoverse_conflicts()"
  )
...
#' @export
print.cytoverse_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(cytoverse_conflict_message(x))
}
